"""
AE encoder → SupCon (Supervised Contrastive) fine-tune → linear probe.

Uses labels: same-class samples are positive pairs, different-class are negatives.
...
from task3_contrastive.model import SegNetEncoderWrapper
...
class SupConLoss(nn.Module):
    ...
    For each anchor, positives = same-class samples + its own augmented view.
    ...
    def forward(self, z1, z2, labels):
        ...
        # For each anchor, compute loss over its positives
        # log_softmax over all non-self entries
...
class SupConPairDataset(torch.utils.data.Dataset):
    """Dataset producing augmented pairs with labels for SupCon."""
...
def train_ae_supcon(cfg):
    ...
    encoder = SegNetEncoderWrapper(segnet, embed_dim=embed_dim).to(device)
    total_params = sum(p.numel() for p in encoder.parameters() if p.requires_grad)
    logger.info(f"Encoder params: {total_params:,}")
    ...
    # Phase 1: SupCon fine-tune
    logger.info("=== Phase 1: SupCon Fine-tune ===")
    best_loss = float("inf")
